...
export default function DoingsCard(props: DoingCardProps) {
    ...
    return (
        <Tilt options={{ max: 45, scale: 1, speed: 450 }}
            className={`
                relative xs:w-[200px] max-w-[12rem] aspect-square
                bg-gradient-to-br from-violet-500/10 via-purple-500/10 to-violet-500/10
                border border-blue-500/10 rounded w-full
            `}>
            <motion.div variants={fadeIn("left", "spring", 0.5 * index, 0.74)}
                ...
                <div className="absolute bottom-1 text-black/80 dark:text-white/80">{doing.title}</div>
            </motion.div>
        </Tilt>
    );
}
...
function DoingTool(props: ImageItemType & {index: number}) {
    ...
    return (
        <motion.div
            className={`
            flex items-center shadow shadow-black/30 dark:shadow-white/20 justify-center flex-col gap-1
            bg-gradient-to-br from-violet-500/20 to-blue-500/20 via-purple-500/70
